...
@app.route('/')
def index():
    return render_template("index.html")  # New: Loads a frontend HTML page
...
def scrape_website():
    ...
    return extracted_data  # Fixed error: return is now correctly placed
...
def generate_response(question):
    ...
    prompt = f"Based on the following structured information about Zhizhen Yang, provide a smooth, well-written paragraph without special formatting that directly answers the user's question: 
{question} Context: {combined_data}"
...
def chat():
    ...
    question = data.get("question", "")  # Fixed: Corrected variable assignment
    if not question:
